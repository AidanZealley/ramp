...
export const RouteSimulationMap = ({
  ...
  onRouteClick,
  presentation,
  riderDistanceMeters: targetRiderDistanceMeters,
  riderGradePercent,
  riderPosition,
  route,
}: RouteSimulationMapProps) => {
  const className = "absolute inset-0 bg-muted"
  const followPosition = follow
  const geojson = route.geojson
  const bounds = route.bounds
  const riderRoutePoints = route.points
  const riderDistanceMeters = targetRiderDistanceMeters
  ...
  useEffect(() => {
    ...
    const map = mapRef.current?.getMap()
    const source = map?.getSource(RIDER_SOURCE_ID) as unknown as
      | GeoJSONSource
      | undefined
